...
6. **新增需求文档**: 必须将确认过的新需求以md格式写入`docs/requirements`
...
* 所有命令从**项目根目录**执行：
  - 后端测试：`pnpm test -- --project backend backend/tests/unit/xxx/xxx.test.ts`
  - 前端测试：`pnpm test -- --project frontend frontend/tests/components/xxx.test.tsx`
  - 构建全量：`pnpm build`（tsc + tsc-alias → vite build frontend）
  - 开发模式：`pnpm dev`（concurrently 启动前后端）
* 测试框架：**vitest**（后端 node 环境，前端 jsdom 环境，根目录 vitest.config.ts 统一配置）
...
- [ ] 新需求 -> 需求以md格式写入`docs/requirements`，允许包含架构设计内容，禁止包含具体实施细节或计划